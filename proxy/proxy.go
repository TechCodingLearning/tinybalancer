...
import (
	...
	urlb "net/url"
	"sync"
	"tinybalancer/balancer"
)

var (
	XRealIP       = http.CanonicalHeaderKey("X-Real-IP")       // 客户端
	XProxy        = http.CanonicalHeaderKey("X-Proxy")         // 反向代理主机
	XForwardedFor = http.CanonicalHeaderKey("X-Forwarded-For") // 前向
)
...
func NewHTTPProxy(targetHosts []string, algorithm string) (*HTTPProxy, error) {
	...
	for _, targetHosts := range targetHosts {
		url, err := urlb.Parse(targetHosts)
		if err != nil {
			return nil, err
		}
...
func (h *HTTPProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	defer func() {
		if err := recover(); err != nil {
			log.Printf("proxy causes panic: %s\n", err)
